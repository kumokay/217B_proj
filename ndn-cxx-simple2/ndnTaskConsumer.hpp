...
#ifndef NDN_TASK_CONSUMER_HPP
#define NDN_TASK_CONSUMER_HPP
...
#define CLASSNAME ndnTaskConsumer
...
public:
      ...
      const ndn::Name& name, int after_ms, int latency)
  {
    ...
    ndnRealApp::scheduleEvent_ExpressInterest(new_name, after_ms, std::bind(&CLASSNAME::onData, this, _1, _2));
  }
private:
  void
  onData(const ndn::Interest& interest, const ndn::Data& data)
  {
    ...
    if (name.getSubName(0, 2).toUri() == "/available/task")
    {
      std::string str_content(data.getContent().value_begin(), data.getContent().value_end());;
      ndnRealApp::scheduleEvent_ExpressInterest(str_content, 0, std::bind(&CLASSNAME::onData, this, _1, _2));
